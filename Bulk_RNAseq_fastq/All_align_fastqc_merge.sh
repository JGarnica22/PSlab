...
#BSUB -q sequential
#BSUB -W 12:00
#BSUB -eo /gpfs/projects/cek26/mouse/reports/Loop_fastqc.err
#BSUB -oo /gpfs/projects/cek26/mouse/reports/Loop_fastqc.out
#BSUB -M 1800
...
echo \#BSUB -q bsc_ls
echo \#BSUB -W 2:00
echo \#BSUB -eo $wd/reports/aligment_counts_$(cut -d'.' -f1 <<< $f).err
echo \#BSUB -oo $wd/reports/aligment_counts_$(cut -d'.' -f1 <<< $f).out
echo \#BSUB -M 1800
echo \#BSUB -n 128
...
R
RPG_list <- list.files(path=paste0(getwd(),"/alignment_counts"), pattern= "*ReadsPerGene*")

for (g in 1:length(RPG_list)){
  x <- read.table(paste0(getwd(), "/alignment_counts/", RPG_list[g]),
                  header = T,
                  sep = "\t",
                  quote = "", 
                  dec = ".")
