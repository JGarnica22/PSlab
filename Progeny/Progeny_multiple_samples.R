#This script is to perform Progeny analysis with multiple samples
...
for (n in c(2:length(pop))) {
  ...
  result1 <- apply(t(progeny_scores_scaled), 1, function(study) {
...
#Checking for differences between the groups:
#We check if the control is different to the treated condition using a linear model:
#1. Stablish control samples, and do a loop in case more than one condition, other than control, is present
for (n in c(2:length(pop))) {
controls <- sample_info$cell_type == pop[1]
study <- sample_info$cell_type == pop [n]
#2. Perform linear regression to calculate enrichment of pathways:
...
for (n in c(2:length(pop))) {
  ...
  dds1 <- DESeq(dds1)
  resLFC <- lfcShrink(dds1, coef=resultsNames(dds1)[n], type="apeglm")
...
write_xlsx(PS2, paste0("output/Progeny_scores_from_DESeq2_", 
                       strsplit(resultsNames(dds1)[n], "_")[[1]][3], "_vs_",
                       strsplit(resultsNames(dds1)[n], "_")[[1]][5],
                       ".xlsx"))
...
progeny2graph <- ggplot(PS2, aes(x=rownames(PS2), y=progenyscores2)) + geom_col(fill="navyblue") +
  ...
  ggtitle(paste0("Progeny analysis based on DEseq analysis ", strsplit(resultsNames(dds1)[n], "_")[[1]][3], " vs ",
                 strsplit(resultsNames(dds1)[n], "_")[[1]][5])) +
...
pdf(file = paste0("figs/Graphbar_progeny_on_DESeq_", 
                  strsplit(resultsNames(dds1)[n], "_")[[1]][3], "_vs_",
                  strsplit(resultsNames(dds1)[n], "_")[[1]][5],
...
pdf(file=paste0("figs/Progeny_score_each_pathway",strsplit(resultsNames(dds1)[n], "_")[[1]][3], "_vs_",
               strsplit(resultsNames(dds1)[n], "_")[[1]][5] ,".pdf"))
