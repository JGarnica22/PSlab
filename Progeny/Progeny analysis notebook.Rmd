---
title: "Progeny analysis notebook"
...
---
This is an [R Markdown](http://rmarkdown.rstudio.com) Notebook. This script is to PROGENy analysis with RNAseq data. It was created for R 3.6.2 version (2019-12-12).Copyright (C) 2020 <br />
# PROGENy ANALYSIS OF RNAseq <br />
This analysis is based on Schubert et al., 2018 [1]<br />
```{r loading packages and data to be used, include=F, warning=T, message=F}
# Load packages:
library(ggplot2)
library(ggrepel)
library(pheatmap)
...
counts <- read.table(paste0("data/",expfile),
                               ...
                               quote = "", 
                               dec = ".")
#Prepare the DESeq2 results table
...
## PROGENY ANALYSIS
```{r statistical progeny analysis, echo=F, warning=F, message=F}
...
```{r generate progeny scores table, echo=F, results='asis' }
kable(results, caption="Progeny scores scaled")
```


**Analysis of pathway scoring sinificance** <br />

 To check significance of progeny pathway scorings,<br />
 we will perform Progeny on several (usually n=1000) permutations<br />
```{r check significance of progeny pathway scorings,echo=F, warning=F, message=F}
#use `runProgenyFast` function:
#'\code{runProgenyFast}
...
## Heatmap with progeny permutations<br /> 
```{r heatmap progeny permutations, echo=F, message=FALSE, warning=F}
#HEATMAP
pheatmap(progeny.permutations, scale = "column")
```
```{r Calculate progeny permutations scores, include=F, warning=F, message=F}
#Checking for differences between the groups:
#We check if the control is different to the treated condition using a linear model:
...
```
```{r generate progeny permutations cable, echo=F, results='asis' }
kable(results1, caption="Progeny permutations scores ")
```
```{r generate progeny permutations scores graph based on p and t value , include=F, warning=F, message=F}
...
```
## Progeny permutations weights<br />
```{r Plot pathways depending on significance and activation ,echo=F, message=FALSE, warning=F}
Progeny_plot
```
# PROGENy analysis based on DESeq2 results
```{r Progeny scores based on DESeq2,  include=F, warning=F, message=F}
...
progeny2graph <- ggplot(PS2, aes(x=rownames(PS2), y=progenyscores2)) + geom_col(fill="navyblue") +
  ...
  theme(panel.background = element_rect(fill = "white",
        ...
        panel.grid.major = element_line(size = 0.05, linetype = 'solid',
                                        colour = "grey"))
```
## Pathways scores based on DESeq2 analysis
```{r generate progeny scores from DESeq2, echo=F, results='asis' }
kable(PS2, caption="Progeny scores from DESeq2")
```

```{r Plot pathways score based on DESeq2 analysis ,echo=F, message=FALSE, warning=F}
progeny2graph
```


## Check in every pathway how genes behave
```{r Plot all pathways gene-specific,echo=F, message=FALSE, warning=F}
...
<br />
## REFERENCES <br />
[1] https://doi.org/10.1038/s41467-017-02391-6
